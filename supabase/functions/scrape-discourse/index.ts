...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await supabase.from("error_log").insert({ function_name: "scrape-discourse", error_message: "Function started (v2)", context: "health-check" });
    ...
    await supabase.from("error_log").insert({
      ...
      error_message: `Done: inserted=${summary.inserted} topics=${summary.topics} fetched=${summary.fetched} classified=${summary.classified} irrelevant=${summary.irrelevant}`,
