-- Drain at */2 with limit=200 and batch_size=40 now that the account is on
-- the paid Gemini tier (Tier 1: 1000 RPM / 10K RPD).
-- Backed by GEMINI_DAILY_REQUEST_LIMIT=5000 / GEMINI_MINUTE_REQUEST_LIMIT=300
-- secrets bumped in Lovable. With ~600 posts/day ingest, this clears any
-- single-scraper-run dump in one pass and shrinks current-day catch-up time
  ...
  PERFORM cron.schedule(
    ...
    format($cron$
      SELECT net.http_post(
        ...
        body := '{"scheduler":"pg_cron","pipeline":"drain-classifications","limit":200,"batch_size":40}'::jsonb
      );
