-- Ensure complaint normalizer exists (was missing from live DB)
...
SET
  classification_status = CASE WHEN sentiment IS NULL THEN 'irrelevant' ELSE 'classified' END,
...
INSERT INTO public.scraped_posts (
  model_id, source, source_url, title, content, content_type, score, posted_at,
  classification_status, classification_attempts, next_classification_at,
  last_classification_error, created_at
)
SELECT
  cq.model_id, cq.source, cq.source_url, cq.title,
  COALESCE(cq.content, cq.full_text, cq.title),
  cq.content_type, cq.score, cq.posted_at,
  'pending', cq.attempt_count, COALESCE(cq.next_attempt_at, now()),
  cq.last_error, cq.created_at
...
RETURNS TABLE(queued integer, retrying integer, failed integer, oldest_queued_at timestamptz, next_attempt_at timestamptz)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public'
AS $function$
  SELECT
    COUNT(*) FILTER (WHERE classification_status = 'pending')::integer,
    COUNT(*) FILTER (WHERE classification_status = 'retry')::integer,
    COUNT(*) FILTER (WHERE classification_status = 'failed')::integer,
    MIN(created_at) FILTER (WHERE classification_status IN ('pending','retry')),
    MIN(next_classification_at) FILTER (WHERE classification_status IN ('pending','retry'))
...
AS $$
  SELECT normalized.category, COUNT(*) AS count
  FROM (
...
AS $function$
  ...
  ranked AS (
    SELECT vs.model_id, vs.score, vs.total_posts, vs.eligible_posts, vs.top_complaint,
           ...
           vs.carried_from_period_start, vs.queued_posts, vs.unclassified_posts,
  ...
  pending_posts AS (
    SELECT model_id, COUNT(*)::integer AS pending_classifications
    FROM public.scraped_posts
...
BEGIN
  FOR job_id IN
    ...
    WHERE jobname IN (
      'run-scrapers-hourly','run-scrapers-hackernews-hourly','run-scrapers-bluesky-hourly',
      'run-scrapers-mastodon-hourly','run-scrapers-twitter-hourly','run-scrapers-reddit-hourly',
      'aggregate-vibes-hourly','reaggregate-vibes-recent',
      'run-scrapers-nightly-reaggregate-0930-utc','run-scrapers-nightly-reaggregate-1030-utc',
      'run-pipeline-3x-daily','drain-classification-queue','drain-queue-trigger'
    )
  LOOP
