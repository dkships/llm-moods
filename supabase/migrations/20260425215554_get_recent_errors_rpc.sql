-- get_recent_errors: SECURITY DEFINER RPC that exposes aggregated error_log
-- entries to the dev-only /admin/scrapers panel. error_log itself is RLS-blocked
-- from anon (per migration 20260422133509), so this is the same pattern as
-- get_scraper_monitor_runs in 20260421193047.
--
-- Filters out the routine "Function started" health-checks and the aggregator's
-- completion log line so the result focuses on actual signal.
