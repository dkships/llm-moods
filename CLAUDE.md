...
Full end-to-end audit. Read-only snapshot pulled 16,496 posts (Feb 15 – May 16) and all 335 daily score rows from public REST. Findings + actions:
...
- **Shared design primitives (Apr 2026 polish pass PRs #5/#7; May 2026 Round 2 PRs #18–#25; Round 3 PRs #27–#36):** `src/components/Surface.tsx` is the canonical card wrapper around the `glass` utility (sizes `default | compact | bare` — `tight` was removed R3-06; `motion="fade"` opts in to `animate-fade-in`, calm `hover:border-border/80` baked in). Surface is one tone only — the `tone="accent"` left-border variant was removed in R2-07. The site has two pill primitives: `src/components/FilterChip.tsx` (interactive pressed/unpressed) replaces ad-hoc filter buttons — one shape (`rounded-md px-3 py-1.5 font-mono text-xs`, the `variant` rect/pill prop was removed in R2-03) with a quiet neutral pressed fill (`bg-foreground/10 text-foreground`), not a primary tint; `src/components/Tag.tsx` (added R3-03) is its display-only sibling for metadata pills — one `shape` (square|pill), one `tone` (neutral|destructive|warning, the colored tones being functional tints with neutral/foreground text), always `text-mono-cap` — it replaced every shadcn `<Badge>` and hand-rolled pill (severity, correlation, research tags, "Updated", translation). `src/components/ModelCard.tsx` (deduped R3-04, `showSparkline` prop) and `src/components/ChatterPost.tsx` (deduped R3-05, `extraMeta` + `hideModel` props) are the single sources for the model card (Dashboard + Index) and chatter row (Dashboard + ModelDetail) — don't re-inline them. `src/components/SectionHeader.tsx` (no `icon` prop as of R2-02; titles are sentence-case as of R3-08) and `src/components/PageHeader.tsx` standardize H2/H1 markup. `src/components/BarList.tsx` (added R2-06) is the canonical labeled-progress list (`{label, value, secondary?}` items, optional `max`, `accent`, `ramp`) — all three ModelDetail right-column lists use it; pass `max={100}` when items are already percentages. Use these instead of writing `glass rounded-xl p-6` or hand-rolled label+bar/pill rows inline. `ScoreMetaBadge` and `DataFreshnessIndicator` were deleted in Round 2 — don't reintroduce them (status/freshness flags fold into the mono-cap meta line as text-tertiary suffixes). The type ladder is 8 rungs (`text-hero` added R3-01: hero / score-xl / score / page / section / body / meta / mono-cap, defined under the ladder comment in `src/index.css`); pick a rung, never a hand-rolled `text-[Npx]` / `text-lg` / `text-xs uppercase tracking-wide`. Sentiment colors derive from the single `SENTIMENT_HSL` constant in `src/lib/vibes.ts`; do not reintroduce hex literals or palette classes (`#EF4444`, `text-red-200`, etc.) for sentiment states. Accent (primary) hue is reserved for: the chart stroke, the hero "bad day" glow, the NavBar wordmark + wave + active-link, and the TrendingComplaints %-change number (FilterChip's pressed fill is neutral, not primary). Severity (StatusCard) and staleness (StalenessBanner) read through `--destructive` / `--warning` *tints* with neutral text, never colored body text (R3-02 / R3-07). The research article surface is a deliberate exception (see Research articles). Aesthetic direction is restraint — one page-level fade per render, no per-section staggers, single calm border-color hover.
