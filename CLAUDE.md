...
- Official status incidents are persisted every 6 h into `vendor_incidents` by two SQL-only pg_cron rows (`vendor-status-request-6h` → pg_net → `vendor-status-ingest-6h`); public read via `get_public_vendor_incidents`. xAI has no feed.
