...
- **Retiring launched models (`isReleasedVersion` in `_shared/rumor-canon.ts`):** the radar is for *unreleased* versions only. Once a version ships, set `released: true` on its `FAMILY_ALIASES` entry (add one if it wasn't tracked yet, e.g. Sonnet 5's entry folds in the "Sonic 5" mis-spelling). That one flag drives the deterministic drop at all three layers — write (`buildContribution`), display (`mergeRumorRows`), and query (`get_public_rumors`'s `version_key NOT IN (…)` list — the one place you also hand-edit per launch). Family-agnostic + a family-stem strip so "Claude Sonnet 5" is caught too. Default-false, so it never wrongly denies a novel leak. Migration `20260701120000_rumors_released_filter.sql` added the RPC gate. There is **no** `is_unreleased` column — the accumulator holds only unreleased rows by construction; the RPC gate + display filter retire ones persisted before launch (they also age out via 21-day decay).
