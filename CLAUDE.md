...
- **Shared design primitives (Apr 2026 polish pass PRs #5/#7; May 2026 Round 2 PRs #18–#25):** `src/components/Surface.tsx` is the canonical card wrapper around the `glass` utility (sizes `default | compact | tight | bare`, `motion="fade"` opts in to `animate-fade-in`, calm `hover:border-border/80` baked in). Surface is one tone only — the `tone="accent"` left-border variant was removed in R2-07. `src/components/FilterChip.tsx` replaces ad-hoc filter buttons; it is one shape (`rounded-md px-3 py-1.5 font-mono text-xs`, the `variant` rect/pill prop was removed in R2-03) with a quiet neutral pressed fill (`bg-foreground/10 text-foreground`), not a primary tint. `src/components/SectionHeader.tsx` (no `icon` prop as of R2-02) and `src/components/PageHeader.tsx` standardize H2/H1 markup. `src/components/BarList.tsx` (added R2-06) is the canonical labeled-progress list (`{label, value, secondary?}` items, optional `max`, `accent`, `ramp`) — all three ModelDetail right-column lists use it; pass `max={100}` when items are already percentages. Use these instead of writing `glass rounded-xl p-6` or hand-rolled label+bar rows inline. `ScoreMetaBadge` and `DataFreshnessIndicator` were deleted in Round 2 — don't reintroduce them (status/freshness flags fold into the mono-cap meta line as text-tertiary suffixes). Sentiment colors derive from the single `SENTIMENT_HSL` constant in `src/lib/vibes.ts`; do not reintroduce hex literals or palette classes (`#EF4444`, `text-red-200`, etc.) for sentiment states. Accent (primary) hue appears in only four UI patterns: the card top bar, the chart stroke, the chip-pressed state, and the hero "bad day" glow. Aesthetic direction is restraint — one page-level fade per render, no per-section staggers, single calm border-color hover.
