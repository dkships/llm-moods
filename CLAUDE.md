...
- **Publishing is a separate step from syncing, and a push alone does NOT reach llmvibes.ai.** Sequence:
  ...
  3. `mcp__lovable__deploy_project` with **only** `project_id` (`94f104f7-b72c-4dc6-b12d-e84aa593cba4`) — passing `name` rewrites the published slug. Goes live in ~45 s. Do not wait for an auto-deploy: Lovable does not republish on a git push (a bot push in Sept 2026 sat unpublished for a week).
...
- Twitter lanes are the documented 5 (since 2026-09-20: ChatGPT isolated; merged Claude/Gemini; Grok isolated; rumor; merged leaker+press roundup). Grok got its own lane because the old merged lane gave it ~2.5 relevant tweets/day; the two `from:` roundups only used ~14 of their 100-tweet floor per run, so merging them was free. Bare `grok` and `@grok` stay out — both matched bot summons ("@grok is this true?"), 100% irrelevant.
- Headline score = latest daily row that is not *today-and-thin*: `get_landing_vibes` skips the current Pacific day while it has `< 5` eligible posts (Grok read "19 · bad vibes · low sample" off one post at noon), and `is_stale` means older than yesterday. The chart still draws today as a dashed provisional tail.
- Official status incidents are persisted every 6 h into `vendor_incidents` by two SQL-only pg_cron rows (`vendor-status-request-6h` → pg_net → `vendor-status-ingest-6h`); public read via `get_public_vendor_incidents`. All four vendors, xAI included.
...
- Classifier model is a pure config flip via `CLASSIFIER_MODEL` (claude-* → Anthropic, gpt-* → OpenAI, else Gemini; keys `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `GEMINI_API_KEY`). Production model is `gpt-6-sol` as of 2026-09-23 (200-post eval vs a `gpt-5.6-sol` reference: complaint category 82.1% vs Terra's 67.5%, ~11% cheaper, but stricter on relevance — 18 wrongly dropped vs Terra's 8) — rollback = `CLASSIFIER_MODEL=gpt-5.6-terra` (previous production, 2026-08-08 to 2026-09-23), or `claude-haiku-4-5-20251001` / `gemini-2.5-flash` (no redeploy, all providers stay live)
...
`.github/workflows/sync-ship-sense.yml` polls github.com/dkships/ship-sense daily, regenerates the snapshot, and **pushes to `main` on its own** — expect bot commits touching `src/data/ship-sense-{snapshot,teaser}.ts` and `public/benchmark/og.png`, and always `git fetch` before local edits. It verifies with lint + test + build before pushing. **A bot push does not reach llmvibes.ai**: Lovable syncs main into the editor but never republishes on its own (docs: "later changes are not automatically pushed live"; verified 2026-09-14, the site sat one commit behind for a week), and the Lovable MCP is OAuth-only so CI cannot publish. Each push opens or updates a `publish-needed` issue — run the publish sequence from the Lovable Workflow section and close the issue.
